...
\usepackage{fontspec} % enable european encoding and font
...
\usepackage[style=authoryear, dashed=false, maxcitenames=2]{biblatex} % better bibliography
\usepackage{float} % allows for [H] option for figures, which actually enforces position in text
\usepackage[cache=false]{minted} % for display of code blocks
\usepackage{hyperref} % for links in general and table of content to heading linking
\usepackage{wrapfig} % enables wrap figures
\usepackage[export]{adjustbox} % enables trimming of graphics
...
\usepackage{tocbibind} % show table of contents in table of contents
